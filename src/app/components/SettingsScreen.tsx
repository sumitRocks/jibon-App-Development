  ...
  return (
    <div className="size-full flex flex-col bg-[#f8f8fa]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              ...
              {section.items.map((item, idx) => (
                ...
                >
                  ...
                  {item.toggle !== undefined ? (
                    <div onClick={(e) => { e.stopPropagation(); setToggles((t) => ({ ...t, [item.key ?? ""]: !t[item.key ?? ""] })); }}>
                      <Toggle val={toggles[item.key ?? ""] ?? false} onChange={() => {}} />
                    </div>
                  ) : (
        ...
        {/* Logout */}
        <button className="w-full py-4 bg-white rounded-3xl border border-red-100 text-red-500 shadow-sm"
